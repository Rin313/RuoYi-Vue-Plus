...
public class ChapterController {
    ...
    @GetMapping("/list")
    public IPage<ChapterVo> list(ChapterQueryBo bo, PageQuery pageQuery) {
        return chapterService.selectPage(bo, pageQuery);
    }

    @SaCheckPermission("system:chapter:query")
    @GetMapping("/{id}")
    public Chapter getInfo(@NotNull(message = "主键不能为空") @PathVariable Long id) {
